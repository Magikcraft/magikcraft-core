...
describe('holograms', function () {
    ...
    it('can create and remove a new Hologram', function () {
        ...
        var atStart = convert_1.default.sizeOf(__1.holograms.getHolograms());
        var h = __1.holograms.createHologram({
            lines: ['Jasmine Unit Test', 'of', 'Holographic Displays'],
            location: location,
        });
        var now = convert_1.default.sizeOf(__1.holograms.getHolograms());
        expect(now).toBe(atStart + 1);
        // Clean-up
        h.delete();
        var final = convert_1.default.sizeOf(__1.holograms.getHolograms());
