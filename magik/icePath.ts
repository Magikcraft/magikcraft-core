...
export function icePath() {
	...
	const eventHandler = events.playerMove(event => {
		...
		if (headBlock.type == Material.ICE) headBlock.setType(Material.AIR)
		if (feetBlock.type == Material.ICE) feetBlock.setType(Material.AIR)
		...
		if (event.player.isSneaking() && !sneakDebounce) {
			...
			if (
				blockBeneath.type == Material.AIR ||
